...
class EZTranslateApp {
    ...
    async generateAPK() {
        ...
        if (generateBtn) {
            ...
            generateBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i><span>Generando APK...</span>';
        }
        ...
        try {
            ...
            this.showAlert('¡APK generado exitosamente!');
            
        } catch (error) {
            console.error('Error generando APK:', error);
            this.showAlert('Error al generar APK. Intenta de nuevo.');
        } finally {
            if (generateBtn) {
    ...
    downloadAPKFile() {
        try {
            ...
            const apkContent = this.createBasicAPK();
            ...
            this.showAlert('Error al descargar APK. Intenta de nuevo.');
        }
    }

    createBasicAPK() {
        // Crear contenido básico del APK
        const apkInfo = `BeDub APK v1.0.0
Package: com.bedub.app
Generated: ${new Date().toISOString()}
URL: ${window.location.origin}

This is a generated APK file for BeDub application.
Install this file on your Android device to use BeDub as a native app.`;

        // Convertir a bytes
        const encoder = new TextEncoder();
        return encoder.encode(apkInfo);
    }
